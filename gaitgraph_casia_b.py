...
class GaitGraphCASIAB(pl.LightningModule):
    def __init__(
        ...
        # v2.0 호환을 위해 step 결과 저장용 리스트 추가
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # test_step과 동일한 로직을 수행하고 결과를 저장
        output = self._get_step_output(batch)
        self.validation_step_outputs.append(output)

    def on_validation_epoch_end(self):
        self._shared_epoch_end_logic(self.validation_step_outputs, print_output=False)
        self.validation_step_outputs.clear()  # 메모리 관리를 위해 리스트 비우기
    ...
    def on_test_epoch_end(self):
        self._shared_epoch_end_logic(self.test_step_outputs, print_output=True)
        self.test_step_outputs.clear()

    def _get_step_output(self, batch):
        """ validation_step과 test_step에서 중복되는 로직을 추출 """
    ...
    def _shared_epoch_end_logic(self, outputs, print_output=True):
        """ 기존 test_epoch_end의 로직을 공통 메소드로 분리 """
        if not outputs:
            return
